...
function exportEntities(passedConfiguration) {
  ...
  createSimlifeJSONFolder(passedConfiguration.application.forSeveralApplications ? configuration.application.name : '');
...
/**
 * Creates the Simlife entity folder, if possible.
 * @param subFolder the folder (to create) in which the Simlife entity folder will be.
 */
function createSimlifeJSONFolder(subFolder) {
  FileUtils.createDirectory(path.join(subFolder, '.simlife'));
}
...
function filterOutUnchangedEntities() {
  for (let i = 0, entityNames = Object.keys(configuration.entities); i < entityNames.length; i++) {
    ...
    if (FileUtils.doesFileExist(filePath)
        && areSimlifeEntitiesEqual(readEntityJSON(filePath), configuration.entities[entityName])) {
