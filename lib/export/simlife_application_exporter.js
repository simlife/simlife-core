...
function setUpApplicationStructure(application) {
  ...
  applicationToExport['generator-simlife'] = applicationToExport.config;
  applicationToExport.entities = application.entities.toArray();
  applicationToExport['generator-simlife'].testFrameworks = application.config.testFrameworks;
  applicationToExport['generator-simlife'].languages = application.config.languages;
...
function setUpArrayOptions(application) {
  application['generator-simlife'].testFrameworks = application['generator-simlife'].testFrameworks.toArray();
  application['generator-simlife'].languages = application['generator-simlife'].languages.toArray();
  return application;
}

function cleanUpOptions(application) {
  if (!application['generator-simlife'].frontEndBuilder) {
    delete application['generator-simlife'].frontEndBuilder;
  }
  return application;
}
...
function writeApplicationFileForMultipleApplications(application) {
  const applicationBaseName = application['generator-simlife'].baseName;
