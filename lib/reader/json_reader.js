...
/* Parse the given simlife app dir and return a JDLObject */
function parseFromDir(dir) {
  ...
  const jdl = Parser.parseServerOptions(Reader.readEntityJSON(`${dir}/.yo-rc.json`)['generator-simlife']);
  const entityDir = `${dir}/.simlife`;
  let isSimlifeDirectory = false;
  try {
    isSimlifeDirectory = fs.statSync(entityDir).isDirectory();
  } catch (error) {
    // .simlife dir doesn't exist'
    return jdl;
  }
  if (!isSimlifeDirectory) {
    throw new BuildException(
      exceptions.WrongDir,
