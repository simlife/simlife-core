  ...
  describe('::parseEntities', () => {
    ...
    context('when parsing app config file to JDL', () => {
      ...
      before(() => {
        ...
        content = JSONParser.parseServerOptions(yoRcJson['generator-simlife']);
      });
    ...
    context('when parsing entities with relationships to User', () => {
      ...
      context('when skipUserManagement flag is set', () => {
        ...
        before(() => {
          ...
          content = JSONParser.parseEntities(entities, JSONParser.parseServerOptions(yoRcJson['generator-simlife']));
        });
...
function readJsonEntity(entityName) {
  return JSON.parse(
    fs.readFileSync(
      path.join('test', 'test_files', 'simlife_app', '.simlife', `${entityName}.json`),
      'utf-8'
    ).toString()
  );
}
...
function readJsonYoFile() {
  return JSON.parse(fs.readFileSync('./test/test_files/simlife_app/.yo-rc.json'));
}
