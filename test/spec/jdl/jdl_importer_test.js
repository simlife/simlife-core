...
describe('JDLImporter', () => {
  ...
  describe('#import', () => {
    context('when not parsing applications', () => {
      ...
      before(() => {
        ...
        filesExist = ENTITY_NAMES.reduce((result, entityName) =>
          result && fs.statSync(path.join('.simlife', `${entityName}.json`)).isFile());
      });

      after(() => {
        ENTITY_NAMES.forEach((entityName) => {
          fs.unlinkSync(path.join('.simlife', `${entityName}.json`));
        });
      ...
      it('exports their content', () => {
        ENTITY_NAMES.forEach((entityName) => {
          const entityContent = JSON.parse(fs.readFileSync(path.join('.simlife', `${entityName}.json`), 'utf-8'));
    ...
    context('when parsing one JDL application and entities', () => {
      ...
      after(() => {
        fs.unlinkSync(path.join('.simlife', 'BankAccount.json'));
      ...
      it('creates the entity folder in the same folder', () => {
        ...
        expect(fs.statSync(path.join('.simlife', 'BankAccount.json')).isFile()).to.be.true;
      });
    });
    context('when parsing JDL applications and exporting them', () => {
      ...
      it('exports the application contents', () => {
        contents.forEach((content) => {
          expect(content['generator-simlife'].jwtSecretKey).not.to.be.undefined;
          delete content['generator-simlife'].jwtSecretKey;
        });
    ...
    context('when parsing multiple JDL files with applications and entities', () => {
      ...
      after(() => {
        fs.unlinkSync(path.join('myFirstApp', '.simlife', 'A.json'));
        fs.unlinkSync(path.join('myFirstApp', '.simlife', 'B.json'));
        fs.unlinkSync(path.join('myFirstApp', '.simlife', 'E.json'));
        fs.unlinkSync(path.join('myFirstApp', '.simlife', 'F.json'));
        fs.unlinkSync(path.join('mySecondApp', '.simlife', 'E.json'));
        fs.unlinkSync(path.join('myThirdApp', '.simlife', 'F.json'));
        APPLICATION_NAMES.forEach((applicationName) => {
          fs.unlinkSync(path.join(applicationName, '.yo-rc.json'));
          fs.rmdirSync(path.join(applicationName, '.simlife'));
      ...
      it('exports the applications', () => {
        APPLICATION_NAMES.forEach((applicationName, index) => {
          ...
          expect(readJSON['generator-simlife'].jwtSecretKey).not.to.be.undefined;
          delete readJSON['generator-simlife'].jwtSecretKey;
      ...
      it('exports the entities for each application', () => {
        APPLICATION_NAMES.forEach((applicationName) => {
          let readJSON = null;
          expect(fs.statSync(path.join(applicationName, '.simlife')).isDirectory()).to.be.true;
          switch (applicationName) {
          case 'myFirstApp': // A, B, E, F
            ENTITY_NAMES.forEach((entityName, index) => {
              readJSON = JSON.parse(
                fs.readFileSync(
                  path.join(applicationName, '.simlife', `${entityName}.json`),
                  'utf-8'
                ).toString());
          ...
          case 'mySecondApp': // only E
            readJSON = JSON.parse(
              fs.readFileSync(
                path.join(applicationName, '.simlife', 'E.json'),
                'utf-8'
              ).toString());
            expect(readJSON.changelogDate).not.to.be.undefined;
            delete readJSON.changelogDate;
          ...
          case 'myThirdApp': // only F
            readJSON = JSON.parse(
              fs.readFileSync(
                path.join(applicationName, '.simlife', 'F.json'),
                'utf-8'
              ).toString());
            expect(readJSON.changelogDate).not.to.be.undefined;
            delete readJSON.changelogDate;
    ...
    context('when choosing \'no\' as database type', () => {
      ...
      after(() => {
        ...
        fs.unlinkSync(path.join('.simlife', 'B.json'));
        fs.rmdirSync('.simlife');
      });
