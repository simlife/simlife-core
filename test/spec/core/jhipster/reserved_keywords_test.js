...
describe('ReservedKeywords', () => {
  describe('::isReserved', () => {
    ...
    context('when passing a valid keyword', () => {
      it('returns false', () => {
        expect(ReservedKeywords.isReserved('ValidKeyword', 'SIMLIFE')).to.be.false;
        expect(ReservedKeywords.isReserved('ACCOUNT', 'JAVA')).to.be.false;
      });
    });
    context('when passing an invalid simlife keyword, no matter the case', () => {
      it('returns true', () => {
        expect(ReservedKeywords.isReserved('Account', 'SIMLIFE')).to.be.true;
        expect(ReservedKeywords.isReserved('account', 'simlife')).to.be.true;
        expect(ReservedKeywords.isReserved('ACCOUNT', 'SIMLIFE')).to.be.true;
        expect(ReservedKeywords.isReserved('ACCOUNT', 'simlife')).to.be.true;
      });
