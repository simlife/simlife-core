...
describe('::exportToJDL', () => {
  ...
  describe('when passing valid arguments', () => {
    describe('when exporting json to entity JDL', () => {
      const jdl = parseFromDir('./test/test_files/simlife_app');
      Exporter.exportToJDL(jdl);
      const input = JDLReader.parseFromFiles(['./simlife-jdl.jh']);
      ...
      ['Country', 'Department', 'Employee', 'Job', 'JobHistory', 'Location', 'Region', 'Task'].forEach((entityName) => {
        previousEntities[entityName] = readEntityJSON(`./test/test_files/simlife_app/.simlife/${entityName}.json`);
      ...
      it('exports it', () => {
        expect(fs.statSync('./simlife-jdl.jh').isFile()).to.be.true;
        expect(newEntities).to.deep.eq(previousEntities);
        // clean up the mess...
        fs.unlinkSync('./simlife-jdl.jh');
      });
    });
  });
});
